...
Contained here:
A) Instructions on choosing appropriate benchmarks – AnchorFinder or otherwise
B) Instructions on preparing those structures – for benchmarking or via AnchoredPDBCreator
C) How to benchmark AnchoredDesign against those structures
D) How to use AnchoredDesign to design interfaces
E) command lines/option files, and discussion of the options

Not contained here:
A) A speck of code – this code is intended to be released with whatever Rosetta release accompanies the PLoSone issue.  The code is all checked in to trunk if you want it in the meantime.  (Note to Sherriff Doug – once you guys decide on distribution, this can change...)
B) Submission scripts for running jobs.  I don't know your architecture.  It's all MPI compatible so it's not hard.

Sort-of contained here:
A) The raw_documentation directory includes copies of the doxygen-style manual documentation for AnchorFinder, AnchoredDesign and AnchoredPDBCreator.  These copies are not guaranteed to be up to date; look in developer trunk or your copy of the code instead.

________OVERVIEW________________________
...
A) one or a few contiguous residues – the protocol can only have one anchor
B) many interactions across an interface – this is the whole point of an anchor
C) not embedded in the middle of a secondary structure element (helix/strand) – the anchor needs to be in a loop because the interface space will be sampled via loop remodeling of the anchor loop
...
_______COMPILING ANCHORFINDER_________________________
...
__________________USING ANCHORFINDER___________________________
...
A) length of windows for consideration - 4 or 5 or 6, etc, contiguous residues at a time?  This flag is -window_size.  I suggest 5 residue windows.
B) What fraction of this window should have loop secondary structure as assigned by DSSP?  -loopness controls this.  It takes it as a decimal between 0-1, I suggest 0.6 (which translates to 3/5 residues for a 5 residue window) to 1 (all residues loop).
C) How many cross-interface interactions per residue should the window have?  I suggest a minimum of 4.  This translates to 20 (redundancy included) cross-interface interactions for a 5 residue window.  By redundancy, I mean residues 43 and 44 on chain A can both interact with residue 234 on chain B and it will count as two interactions.  Specify this with -nbrs_per_residue.
D) What file name should the good interactions be printed to?  I leave it as an exercise to the reader to pick their own file name.  Specified with -bestoutfile; defaults to goodfile.out.
...
_________________INTERPRETING ANCHORFINDER RESULTS__________________
...
________________CHOOSING LOOP & ANCHOR - BENCHMARKING_______________________
...
_________________CHOOSING A SYSTEM AND ANCHOR – DESIGN__________
...
A) have crystal structures, and
B) are related to your biological problem.

Choosing an anchor then requires:
C) a cocrystal of your target with some partner from which to source the anchor.
...
_____CHOOSING A SCAFFOLD, DESIGN POSITIONS, AND LOOPS – DESIGN____________
...
A) flexible, mutateable surface loops (for AnchoredDesign to modify)
B) experimentally tractable (hey, your funeral if it's not)
C) whatever other functionality you need for your desired design
...
____________________CHOOSING LOOP LENGTHS AND ANCHOR POSITION___________
...
_________________USING ANCHOREDPDBCREATOR_____________________
...
A) The target structure, as a PDB, with the partner removed
B) The anchor structure, drawn from the cocrystal with the target, containing ONLY the residues being used as an anchor, as a PDB
C) The scaffold structure, as a PDB, with loop residues added/deleted as desired
D) A scaffold_loop specification, which declares which residues in the scaffold are flexible and where the anchor insertion should occur.
...
_________________RUNNING ANCHOREDDESIGN_______________________
...
A) The anchor file specifies which residues form anchor.
B) The PDB file is the pdb.
C) loopsfile_extended is for benchmarking – the extension column is true, which tells AnchoredDesign to forget the starting loop conformation before sampling
D) loopsfile_native tells AnchoredDesign NOT to forget the starting loop conformation – this is probably what you would use for design, although there is no reason you can't reject the starting loop conformation for design.  (This is cheating for benchmarking, but useful for making relaxed natives)
E) options is the command line options file.  The active options are for a simple “does it run” test; parameters for a longer running “real” test are included.
F) A resfile is necessary if you wish to design, and is only active in that option set.  

____________INTERPRETING ANCHOREDDESIGN FOR BENCHMARKING___________________
...
____________INTERPRETING ANCHOREDDESIGN FOR DESIGNS______________

(This section applies to the design case only)

In the design case, the other fields of the AnchoredDesign output come in to play. There are three classes of output:
A) scorefunction terms
B) LoopAnalyzerMover output,
C) InterfaceAnaylzerMover output.
...
_________________INTERFACEANALYZERMOVER__________________________
