...
This can be done with the following two steps:
    ...
    b) Translate top scoring pairs into Rosetta specific distance restraints
...
Alternatively, fragments can be generated using the webserver Robetta (available at http://www.robetta.org/). 
...
All runs in our manuscript have been set up with the CSRosetta Toolbox (available at http://www.csrosetta.org/). In case, the Toolbox is not available, a folder containing all necessary flag files is provided. Both ways (with and without CSRosetta toolbox) to set up a RASREC run will be described below.
...
# converged  distances in low_30.pdb translated to strict bounded potentials around the average distance
restraints_converged_distances.cst 
# additional restraints from the contactmap that do not completely disagree with the previous results. Here a more widely bounded potential is used.
