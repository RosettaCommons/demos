...
# Your name: Andrew Leaver-Fay
# Protocol Name: Heterodimeric Antibody Design using Multistate Design Code
# Brief Description:
#
...
# [Your multistate design project here], Andrew Leaver-Fay in the Kuhlman lab @ UNC, 2010, Session 10, Thursday
# Abstract
...
#
# -ms::generations <int> I have found that the number of generations should be ~ 15 x N where N
...
#
# -ms::pop_size <int> I have found that, using ms::generations <15*N>, the population size of 100 is good.
#
# -ms::fraction_by_recombination <float> The fraction of crossover events; the fraction mutated by 
...
#
# -ms::numresults <int> The number of results to output, sorted by increasing (worsening) fitness.
...
# assignment).
# E.g. If you used the "vmin" function to select the state from a state vector with the lowest energy,
# then only the state with the lowest energy is output.
#
# Output pdbs are named with "msd_output_", the rank of the source entity (1..numresults), the name for the state
# variable or the state-vector variable (this name comes from the fitness file) and a ".pdb".
