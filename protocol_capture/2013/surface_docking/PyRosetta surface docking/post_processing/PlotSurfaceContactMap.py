...
def MAIN():
    ...
    Index = [arange(0,total_res-1,1)+1,arange(0,30,1)]
    ...
    num_pdb = sum(1 for line in open('PDBs','r'))
    ...
    O.write('%0s \n' %('set cbtics (0, ' + str(num_pdb) + ') nomirror scale 0'))
    ...
    # CHANGE TITLE HERE <<<<<<<<<<<<<<<<
    O.write('%0s \n' %('set title "TITLE" font "VeraBd, 20"')) 
...
plot = '~/Applications/bin/gnuplot SurfaceContactMap.gnuplot'
os.system(plot)
