...
alter_conform = 'A'
if argv.count('-alter_conform'):
    pos = argv.index('-alter_conform')
    del( argv[ pos ] )
    alter_conform = argv[ pos]
    del( argv[pos] )

if(alter_conform != 'A' and alter_conform != 'B'):
	print "alter_conform != 'A' and alter_conform != 'B'"
	assert(False)
...
for pdbname in pdbnames:
    ...
    outfile = outfile.replace( '.pdb', '_RNA_' + alter_conform + '.pdb').replace('.gz','');
