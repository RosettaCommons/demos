...
1. Preparation for running MembraneAbinitio
    ...
    1. Prepare your working directory. You will work in this directory for the rest of the tutorial.
        1. Create a directory in the protein_folding/MembraneAbinitio directory called my_files and switch to that directory. 
                
                cd ~/rosetta_workshop/tutorials/protein_folding/MembraneAbinitio/
                mkdir my_files
                cd my_files
    ...
    1. Save your protein sequence to a file using FASTA format.
        ...
        **~/rosetta_workshop/tutorials/protein_folding/MembraneAbinitio/input_files**  
        **directory.**
        ...
        1. Get the sequence in FASTA format from NCBI.
            ...
            1. Save the file as BRD4A.fasta to the  
            ~/rosetta_workshop/tutorials/protein_folding/MembraneAbinitio/my_files directory.
    ...
    1. Generate the LIPS lipophlicity file [http://tanto.bioengr.uic.edu/lips/](http://tanto.bioengr.uic.edu/lips/) (Adamian and Liang, BMC Struct. Biol., 2006).
        - *Note: Can only generate this file if have BLAST and NR database installed.  To use the LIPS server, you must have a multiple sequence alignment.  We will not run this script during the workshop!  See the example below:*
                ...
                ~/rosetta_workshop/rosetta/main/source/src/apps/public/membrane_abinitio/run_lips.pl\
                BRD4A.fasta BRD4A.span  /blast/bin/blastpgp /nr_database\
                ~/rosetta_workshop/rosetta/main/source/src/apps/public/membrane_abinitio/alignblast.pl
    ...
    1. Prepare fragment libraries.
        ...
        **~/rosetta_workshop/tutorials/protein_folding/MembraneAbinitio/input_files directory.**
...
1. Running the MembraneAbinitio application.
    ...
    1. Go to the ~/rosetta_workshop/tutorials/protein_folding/MembraneAbinitio/my_files directory
    1. Execute the following command line.  

            ~/rosetta_workshop/rosetta/main/source/bin/membrane_abinitio2.default.linuxgccrelease\
            -database ~/rosetta_workshop/rosetta/main/database @BRD4_mem_abrlx.options
        
        - NOTE:  This will take 3-5 minutes per structure
1. Analyze your data.

    **Example data is provided for you in the**     
    **~/rosetta_workshop/tutorials/protein_folding/MembraneAbinitio/example_data/**  
    **directory.**
    ...
    1. For practice, we will be analyzing data that has already been generated.
        ...
        1. Copy the files from the example_data directory.
                ...
                cp ~/rosetta_workshop/tutorials/protein_folding/MembraneAbinitio/example_data/* .
    ...
    1. Combing and extracting silent files and extracting PDBs of membrane proteins is slightly different than for soluble proteins, but is a very similar process as that described in the de novo folding tutorial. To combine silent files: 
            ...
            -database ~/rosetta_workshop/rosetta/main/database -in:file:silent BRD4A*.out\
            -in:file:silent_struct_type binary -in:file:residue_type_set centroid\
            -in:file:spanfile  BRD4A.span\
            -score:weights ~/rosetta_workshop/rosetta/main/database/scoring/weights/score_membrane.wts\
            -out:file:silent BRD4A_mem_abrlx_all.out -out:file:silent_struct_type binary\
            -out:file:residue_type_set centroid
        
        1.  Find the lowest-scoring models.
    
                python ~/rosetta_workshop/rosetta/tools/protein_tools/scripts/score_scatter_plot.py\
                --x_axis rms_TM --y_axis score --silent BRD4A_mem_abrlx_all.out\
                BRD4A_mem_abrlx_all_score_vs_rmsd.table
        ...
        1.  To extract PDBs:  
                ...
                -in:file:silent BRD4A_mem_abrlx_all.out -in:file:tags S_00000058_1\
                -in:file:silent_struct_type binary -in:file:residue_type_set centroid\
                -in:file:spanfile BRD4A.span\
                -score:weights ~/rosetta_workshop/rosetta/main/database/scoring/weights/score_membrane.wts\
                -out:output -out:pdb -out:file:residue_type_set centroid\
                -in:file:tags S_00000044_1_0001_0001 S_00000044_1_0001 S_00000077_1_0001\
                S_00000130_1_0001 S_00000144_1_0001_0001 S_00000144_1_0001 S_00000168_1_0001_0001\
                S_00000168_1_0001 S_00000187_1_0001 S_00000247_1_0001
    ...
    1.  Generate score vs. RMSD plots etc. The scripts to make an XY-scatter plot for membrane proteins are called score_vs_rmsTM.R and score_vs_rmsTM_low.R. These scripts are found in the ~/rosetta_workshop/tutorials/protein_folding/scripts directory.  If you want to rescore your models and compute the RMSD against the lowest-scoring model, repeat the previous step with the following changes: 
            ...
            ~/rosetta_workshop/rosetta/main/source/bin/score_jd2.default.linuxgccrelease\
            -database ~/rosetta_workshop/rosetta/main/database -in:file:native S_00000044_1_0001_0001.pdb\
            -in:file:silent BRD4A_mem_abrlx_all.out -in:file:silent_struct_type binary\
            -in:file:residue_type_set centroid -in:file:spanfile BRD4A.span\
            -score:weights ~/rosetta_workshop/rosetta/main/database/scoring/weights/score_membrane.wts\
            -out:file:residue_type_set centroid -out:file:silent BRD4A_mem_abrlx_all_rescore.out\
            -out:file:silent_struct_type binary -evaluation:rmsd NATIVE TM_low BRD4A_TM_rms.txt

    1. *OPTIONAL:  Relaxing Membrane Proteins in Rosetta (will not be covered in tutorial)*
         1. *The options and command line for performing full atom refinement of a membrane protein in ROSETTA is provided in the ~rosetta_workshop/tutorials/protein_folding/MembraneAbinitio/input_files/mem_rlx directory. The example output can be found in*  
              *~/rosetta_workshop/tutorials/protein_folding/MembraneAbinitio/analyze_membrane_relax.*  
...
1. Folding membrane proteins with the topology broker with restraints
    ...
    **~/rosetta_workshop/tutorials/protein_folding/MembraneAbinitio/membrane_broker_with_constraints.**
