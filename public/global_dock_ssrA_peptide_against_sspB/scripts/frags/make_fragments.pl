...
my $pathToDemo="/vol/ek/ravehb/rosetta/RosettaCon2011/demos/global_dock_ssrA_peptide_against_sspB/";

# psi-blast
#my $BLAST_BIN_DIR = "$shareware_dir/new_blast/blast-2.2.12/bin";
my $PSIBLAST = "$scratch/src/blast/blastpgp";     # PSI-BLAST (duh.)
my $NR = "$scratch/genomes/nr";                # nr blast database filename
my $VALL_BLAST_DB = "$scratch/$fragsDir/nnmake_database/vall.blast.2006-05-05";        # vall blast database filename (cvs respository 'nnmake_database')
my $BLOSUM = "$scratch/$fragsDir/nnmake_database/";      # BLOSUM score matrices directory (cvs repository 'nnmake_database')

# psipred
my $FILTNR = "$scratch/genomes/filtnr";        # filtnr blast database filename
my $MAKEMAT = "$scratch/src/blast/makemat";       # makemat utility (part of NCBI tools)
my $PSIPRED = "$scratch/src/psipred/bin/psipred";       # psipred
my $PSIPASS2 = "$scratch/src/psipred/bin/psipass2";     # psipass2 (part of psipred pkg)
my $PSIPRED_DATA = "$scratch/src/psipred/data";         # dir containing psipred data files.
