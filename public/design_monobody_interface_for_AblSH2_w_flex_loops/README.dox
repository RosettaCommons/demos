METHOD 1
Protocol: RosettaRemodel
Documentation: 
https://wiki.rosettacommons.org/index.php/Remodel

Requirement:
svn co https://svn.rosettacommons.org/source/trunk/rosetta_scripts/remodel

Step 1: Preparing the PDB
Take the PDB 3k2m.pdb. Select chain B and C.
Get rid of HETATM. 
...
Let's say the PDB is 3k2m_bc_relax.pdb
Delete the Chain ID column

Step 2: Generating BluePrint
./getBluePrintFromCoords.pl -pdbfile 3k2m_bc_relax.pdb > test.blueprint

Bluprint file has the information to direct the protocol on which residue to design and remodel.

Step 3: Editing BluePrint
The interface on the monobody is Residue 182-187. We decided to rebuild with 1 residue insertion starting at 183. The blue print has to be modified in the following way
...
178 G .
179 E .
180 D .
...
"0 x L" will mean eXtension and the secondary structure assined for the inserted region in Loop.
Column 1 is the residue postion
Column 2 is the residue identity
Column 3 is the backbone behavior 

Step 4: Running the commandline
...
METHOD 2
We can use a loopmodel with fragmet files and fixBB design with a resfile to design the monobody part of interface.

Step1: Preparing the starting PDB
Same as Step 1 in METHOD 1.
We do not need to get rid of the Chai ID column in this case.
The relaxed PDB (3k2m_bc_relax.pdb) will be the input PDB.

Step 2: Creating fragment libraries
Take the fasta file of 3k2m_bc_relax.pdb. Create  fragment libraries of sizes 9 and 3 locally or through Robetta Server.

Step 3: Other input files
a loop file:
3k2m.loop_file
...
The ctual experiment can have 1000-10000 nstruct.

Step 4: FixBB design with a resfile
...
Step 5: Select PDBs based on total_score

Step 6: Optional run for binding energy.
