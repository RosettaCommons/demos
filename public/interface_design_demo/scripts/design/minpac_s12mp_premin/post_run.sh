...
jobname=; echo "run.sh needs to be executed before post_run.sh"; exit
...
# Get CPU_hours
# The output file listing CPU time is LSF system dependent, this is what I use:
# CPU_secs=$(grep -m 1 'CPU time' ${jobname}* | sed 's/^.*CPU time[^0-9]*\([0-9]\+\(\.[0-9]\+\)\) sec.*$/\1/g')
# CPU_hours=$(echo "scale=3; ${CPU_secs}/3600" | bc)
CPU_hours=$(echo "To report CPU hours, open post_run.sh and uncomment the" \
    "lines under \"# Get CPU_hours\" and remove the line that assigns this" \
    "string. The logic for getting CPU_hours from output files depends on" \
    "your LSF system.")
