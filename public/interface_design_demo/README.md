interface_design_demo

Author: Kevin Houlihan (khouli@unc.edu)

## Need to fix broken link to example_outputs/minpac_optE_premin/selected_chains.list
## Need to fix broken link to example_outputs/minpac_s12mp_premin/selected_chains.list
## Need to fix broken link to example_outputs/pacrot_s12p_nomin/selected_chains.list
## also broken link scripts/design/minpac_optE_premin/selected_chains.list
## and this one /scripts/design/minpac_s12mp_premin/selected_chains.list -> ../../../inputs/selected_chains/selected_chains.list
## and this one scripts/design/pacrot_s12p_nomin/selected_chains.list -> ../../../inputs/selected_chains/selected_chains.list
...
@1 Executive Summary
@2 Setup
@3 Running Design Protocols
@4 Analysis
@5 Example outputs
@6 Where to change parameters
@7 Troubleshooting
@8 Epilogue
...
-The relative paths downwards from interface_design_demo/ have not been changed
    e.g. scripts within the demo can access other scripts within the demo at
    their expected paths
-Your rosetta path contains:
    rosetta_database/
        referenced by design scripts, e.g. pacrot_s12p_nomin.sh (run by run.sh)
    rosetta_source/bin/sequence_recovery.linuxgccrelease
        referenced by scripts/analyze/seqRec.sh (run by infpro.sh)
    rosetta_source/bin/rosetta_scripts.mpi.linuxgccrelease
        referenced by design scripts
        referenced by scripts/analyze/seqRec.sh (run by infpro.sh)
...
@7 Troubleshooting
==================
PMGR_COLLECTIVE ERROR following post_run.sh:

    Fix the path to the sequence_recovery binary in
    interface_design_demo/scripts/analyze/seqRec.sh
