abinitio structure prediction using chemicalshift generated fragments, NOE distance restraints and RDC restraints
**************************************************************************************************************
Author: Lei Shi
...
If you want to use non-linear-square fitting with Da or R fixed, use "-rdc:fit_method nls -rdc:fixDa 12.6 -rdc:fixR 0.42"
...
You should also use NOE constraint energy and RDC energy as a criteria to select structures. Example is only provided for total score.

8.2
cull_silent.pl csrosetta_noe_rdc.out “score < cutoff”
above this will produce csrosetta.select.silent which contains the lowest total energy 10 models.
  
8.3 extract pdbs from silent files for a given tag in the silent file
...
8.7 To see how NOE constraints are satisfied by a model:
Rosetta/main/source/bin/r_cst_tool.linuxgccrelease -database Rosetta/main/database/ -in:file:s lowscore_1.pdb -cst_file rosetta_inputs/NOE.cst
r_cst_tool is a pilot program by Oliver Lange in Rosetta/main/source/src/apps/pilot/olli/

8.8 To see how rdc constraints are satisfied by a model
