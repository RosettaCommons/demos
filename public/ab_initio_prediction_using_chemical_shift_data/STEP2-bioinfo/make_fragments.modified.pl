...
#  CVS information:
#  $Revision: 11090 $
#  $Date: 2006-11-02 12:17:28 -0800 (Thu, 02 Nov 2006) $
#  $Author: dekim $
...
#   $BAKER_HOME/src/nnmake_new/
#
#  e.g. in .cshrc:  setenv BAKER_HOME /work/dylan
#       in .bashrc: export BAKER_HOME=/work/dylan
#
...
$src_dir = '/work/tex/src';
$shareware_dir = $src_dir;# . '/shareware/';
$scratch = "/scratch/ROSETTA/";

# psi-blast
#my $BLAST_BIN_DIR = "$shareware_dir/new_blast/blast-2.2.12/bin";
my $PSIBLAST = "$shareware_dir/blast/bin/blastpgp";     # PSI-BLAST (duh.)
#my $NR = "/scratch/USERS/casp9/nr/nr/nr";                # nr blast database filename
my $NR = "/scratch/ROSETTA/genomes/nr";                # nr blast database filename
...
$VALL_BLAST_DB = "/work/rvernon/yangshen/old_vall/vall.blast.2008.apr24";        # vall blast database filename (cvs respository 'nnmake_database')

my $BLOSUM = "$scratch/nnmake_database/";      # BLOSUM score matrices directory (cvs repository 'nnmake_database')

# psipred
#my $FILTNR = "/scratch/USERS/casp9/nr/nr/filtnr";        # filtnr blast database filename
my $FILTNR = "/scratch/ROSETTA/genomes/filtnr";
my $MAKEMAT = "$shareware_dir/blast/bin/makemat";       # makemat utility (part of NCBI tools)
my $PSIPRED = "/work/rvernon/src/psipred_2.61/bin/psipred";       # psipred
my $PSIPASS2 = "/work/rvernon/src/psipred_2.61/bin/psipass2";     # psipass2 (part of psipred pkg)
my $PSIPRED_DATA = "/work/rvernon/src/psipred_2.61/data";         # dir containing psipred data files.

# prof
my $PROF = "$shareware_dir/shareware/prof/run_prof.py";
...
my $VALL = "/work/rvernon/yangshen/old_vall/";          # dir containing vall database (cvs repository 'nnmake_database')
my $VALL2 = "/scratch/USERS/casp9/";             # alt dir containing vall database (cvs repository 'nnmake_database')
my $VALL_NAME = "vall.dat.apr24";                 # filename of vall (vall.dat.<id> and vall_cst_coord.dat.<id> must exist)
my $NNMAKE = "/work/rvernon/nnmake_new/pNNMAKE.9and3.gnu";                         # nnmake binary  (cvs respository 'nnmake')
...
my $JUFO = "$shareware_dir/shareware/jufo/molecule.exe";                              # jufo executable

# sam (secondary structure prediction software)
my $SAM_target99		= "$shareware_dir/shareware/sam3.5.i686-linux/bin/target99";	# sam target99 executable
my $SAM_uniqueseq		= "$shareware_dir/shareware/sam3.5.i686-linux/bin/uniqueseq";	# sam uniqueseq executable
my $SAM_predict_2nd_dir	        = "$shareware_dir/shareware/sam.predict2nd/";			# sam predict-2nd directory
my $SAM_predict_2nd		= "$shareware_dir/shareware/sam.predict2nd/predict-2nd";	# sam predict-2nd executable
...
#
# Try and run PROF.
# "Who knows what this crazy perl is doing?"
#    - Phil Bradley
if ( $prof ) {
    unless (&fileExist("$id$chain.prof_rdb")) {
        print "running prof.\n";
        &run("$PROF $file ss_blast $id$chain.prof_rdb",("$id$chain.prof_rdb"));
    }
}
##########
#
