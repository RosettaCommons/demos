...
# usage: cluster.sh <pdb name> <top x structures> <radius> <score.sc> <native.pdb> <list structures> <stype=column number of score according to which to select the top x structures>
# e.g. cluster.sh 1AWR 20 1 score.sc native.pdb structures_list.txt 35 ---> clusters top 20 structures by reweighted score with 1A peptide bb rms radius.
...
#send clustering run
if (! -e c.0.0.pdb ) then
	$PATH_TO_EXE/cluster.linuxgccrelease -in:file:l $decoys -in:file:silent_struct_type binary -database $PATH_TO_DB -cluster:radius $actualR -in:file:fullatom -tags `cat $score | sort -nrk $stype | tail -$top | awk '{print $(NF-1)}'` -silent_read_through_errors >! clog
endif
