...
def setup_amino_acid(atoms):
    ...
    if not all( name in atom_names for name in atom_prio.iterkeys() ):
        raise ValueError("Input is missing essential atom(s) for amino acids: %s" % ", ".join(atom_prio.iterkeys()))
    # Make N the root
...
def write_ligand_pdb(f, molfile_tmpl, molfile_xyz, resname, ctr=None, chain_id='X'):
    ...
    for frag_id in frag_ids:
        ...
        for atom_tmpl in atoms:
            ...
            if atom_tmpl.elem != atom_xyz.elem:
...
and for visualizing exactly what was done to the ligand.
    ...
    parser.add_option("-a", "--amino-acid",
        default=None,
        help="set up params file for modified amino acid; .mol2 only; edit chis afterward",
        metavar="ALA"
    )
    ...
    if options.amino_acid is not None: setup_amino_acid(m.atoms)
    net_charge = 0.0
