...
Written by Ben Stranges (stranges@unc.edu)
...
In reality you'll probably want to use -l instead of -s to pass a bigger list of pdbs to look for exposed strands in. This is just an example that will work.
...
If you need to run a bunch of these from the output of the exposed strand finder I have provided a script that reads a file (runner_input).  This file should be multiple lines instead of just the one here.  It's structure is /path/to/pdb/1av3.pdb chainletter betastart betaend
...
3) Homodimer design
The next step is to take the output from above and make the files you need for symmetry. To do this you will need to use the symmetry script as so:
...
However, the only one you really need is 2a7b_mpm_A806_anti_wind_1_step_1_INPUT.pdb and symmdef so I removed the rest. Now you are ready for the full design runs. I suggest using mpi compiled executables but the command line below is general. 
Run this command
