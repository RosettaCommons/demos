#!/usr/bin/perl -w 
###############################################################################
# written by Bjorn Wallner and modified by Davide Sala                        #
# Script takes transmembrane topology predicted by TOPCONS server             #
# (http://octopus.cbr.su.se/) and converts it to *.span format for input      #
# to rosetta-membrane ab initio protocol                                      #
###############################################################################

if ($#ARGV < 0) {
    print STDERR "usage: $0 <TOPCONS topology file>\n";
    exit -1;
}
...
{
    print "TM region prediction for $key predicted using TOPCONS\n";
